...
const Page:NextPage = () => {
    ...
    const searching = (data:any) => {
      return data.filter(
      (item:any) =>
        item.category.toLowerCase().includes('historia') &&
        ...
        <main>
          <Cards>
          {
              ...
              <Card
                ...
                title={title}
              />) :
              articles.filter((item:any) => item.category.includes('historia')).slice(0).reverse().map(({date,excerpt,image,path,tags,title}:i,key:number) =>
              <Card
                content={excerpt}
                date={date}
                image={image}
