...
const Page:NextPage = () => {
    ...
    const {seo,hero} = blog.positioning;
        ...
        <main>
          <Cards>
          {
              ...
              <Card
                ...
                title={title}
              />) :
              articles.filter((item:any) => item.category.includes('seo')).slice(0).reverse().map(({date,excerpt,image,path,tags,title}:i,key:number) =>
              <Card
                content={excerpt}
                date={date}
                image={image}
